...
def even(some_not_even_list):
  if(type(some_not_even_list) == type([])):
...
def get_ages(some_years_of_birth):
  if type(some_years_of_birth) == type([]):
...
def count_symbols(some_string):
  ...
  for letter in some_string:
    if letter not in dictCountLetter.keys():
      dictCountLetter[letter] = 1
    else:
      dictCountLetter[letter] += 1
  return dictCountLetter
